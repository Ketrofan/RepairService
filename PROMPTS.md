...
### 03.03.2026 19:28 
**User request (full):**
Проверка race_test.sh: сначала не было XSRF, затем 419 на login.
Доработали скрипт: GET /login → парсинг `_token` → POST /login → GET /master/requests → параллельные take.
Итог: реальная гонка подтверждена (200/409).
